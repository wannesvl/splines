...
\tikzstyle{tech} = [rectangle, rounded corners,
                    fill = black!10,
                    draw = red, thick,
                    minimum height = 14cm, minimum width = 15cm]
...
\node[draw=red, fill=red!20, rounded corners, very thick, anchor=north, minimum height=17cm, minimum width=35cm] at (0,2) (tech){};%
\node[anchor=north] at ($(tech.north)+(0,-0.4)$) {\textbf{\LARGE \textcolor{red}{ENABLING TECHNOLOGIES}}};%
...
\item positivity on non-hyperrectangular domains
    ...
    \item relation with Polya's relaxation for polynomials on convex polyhedrons
    \end{itemize}
\item assessing conservatism of the constraints + strategy for local refinements
\item rational constraints and \textsc{nurbs}    
\item dealing with complex variables (e.g. in unit disc or \textsc{rhp})
    \begin{itemize}\vspace*{-4pt} \itemsep 0pt
...
\item relation with other nonlinear interior point solvers
\end{itemize}
...
\node[draw=blue, fill=blue!30, opacity=0.5, rounded corners, very thick, anchor=north, minimum height=24.5cm, minimum width=33cm] at ($(tech.south)+(-11,-3)$) (mot){};%
\node[anchor=north] at ($(mot.north)+(0,-0.4)$) {\textbf{\LARGE \textcolor{blue}{\parbox{20cm}{OPTIMAL MOTION PLANNING for DIFFERENTIALLY FLAT SYSTEMS}}}};%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\node[draw=green, fill=green!30, opacity=0.5, rounded corners, very thick, anchor=north, minimum height=24.5cm, minimum width=44cm] at ($(tech.south)+(16.5,-5)$) (rob){};%
\node[anchor=north] at ($(rob.north)+(0,-0.4)$) {\textbf{\LARGE \textcolor{green}{ROBUST OPTIMIZATION}}};%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\node[app12, anchor=north] at($(mot.north)+(11,-3)$) (flat_free){%};%
...
\begin{itemize}\vspace*{-4pt} \itemsep 0pt
\item conservatism of parametrization (versus conservatism of constraints) %
\item convergence to true solution?
\item infeasible parameter values
\end{itemize}
...
node[black,near start] {\large\textbf{\hspace*{2cm}solving structured control problems}};%
...
\draw[ultra thick, black!40, ->] ($(nlp.south)$) .. controls ($(nlp.south)+(0,-3)$) and ($(sysapp.west)+(-3,3.1)$) .. ($(sysapp.west)+(0,3.1)$)%
node[black,midway] {\large\textbf{solving \textsc{bmi}s}};%

\draw[ultra thick, black!40, ->] ($(parapp.east)+(0,-2)$) .. controls ($(parapp.east)+(3,-2)$) and ($(sysapp.west)+(-3,2.9)$) .. ($(sysapp.west)+(0,2.9)$)%
node[black,midway] {\large\textbf{solving \textsc{bmi}s}};%
