...
\newcommand{\Ppar}{{\bm{\theta}}}                   % capitalized parameter ??? should we make this bold ???
...
\item\label{ass_pdep} The problem data depend polynomially on $\ppar$. That is, $\calF(\ppar)$, $g(\ppar)$ and $h(\ppar)$ can be written as
    \[ \calF(\ppar) = \sum_{\alpha\in\Alpha^\calF} \calF_\alpha \, \ppar^\alpha \,,\qquad%
       g(\ppar)     = \sum_{\alpha\in\Alpha^g}         g_\alpha \, \ppar^\alpha \,,\qquad%
       h(\ppar)     = \sum_{\alpha\in\Alpha^h}         h_\alpha \, \ppar^\alpha \,,%
    \]
...
\item\label{ass_feas} For all $\ppar\in\Ppar$ both the primal program $\Pi(\ppar)$ and the dual program $\Delta(\ppar)$ are strictly feasible. That is: $\Ppar\subset\Pstrfeas$ and $\Ppar\subset\Dstrfeas$.
\end{enumerate}
...
In Section~\ref{subsec:extensions}, Assumptions~{\it\text{A}\ref{ass_pdep}.} and {\it\text{A}\ref{ass_feas}.} will be relaxed.
...
Assumption~{\it\text{A}\ref{ass_pdep}.} requires $F^1(\ppar)$, \ldots, $F^n(\ppar)$, $G(\ppar)$ and $h(\ppar)$ to be polynomial functions. To derive the polynomial coefficients of $\calF(\ppar)$, let $\Alpha^\calF$ group the multi-indices of all monomials in $F^1(\ppar)$, \ldots, $F^n(\ppar)$, such that we can write all these matrices as 
\[  F^i(\ppar) = \sum_{\alpha\in\Alpha^\calF} F^i_\alpha \,, \ppar^\alpha\,,\qquad i=1,\ldots,n\,.%
\]
Then, $\calF(\ppar)$ can be written as in {\it\text{A}\ref{ass_pdep}.} with
\begin{align*}
...
\end{gather}
\end{subequations}
where $\cx\in\X^{n_x}$ and $\cy\in\Y^{n_x}$, with $n_x=|\Alpha^x|$ and $n_y=|\Alpha^y|$. The images of $\hat{x}(\cx)$ and $\hat{y}(\cy)$ for a particular $\theta$ are indicated by $\hat{x}(\cx,\ppar)$ and $\hat{y}(\cy,\ppar)$, respectively.
...
\]
By solving $\hat{\Pi}$ and $\hat{\Delta}$ the coefficients $\cx_\feas$ and $\cy_\feas$ are computed that minimize the average gap over $\Ppar$:
