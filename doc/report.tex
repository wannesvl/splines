...
\newcommand{\Ppar}{\Theta}                          % capitalized parameter ??? should we make this bold ???
...
\newcommand{\Pfeas}{\Ppar_{\text{feas}}^\Pi}        % set of parameters for which the primal is feasible
\newcommand{\Dfeas}{\Ppar_{\text{feas}}^\Delta}     % set of parameters for which the dual is feasible
% Infinite dimensional reformulation
\newcommand{\Xm}{\bm{\mathcal{X}}_{\Theta}}         % set of continuous functions \Ppar -> \X
\newcommand{\Ym}{\bm{\mathcal{Y}}_{\Theta}}         % set of continuous functions \Ppar -> \Y
% Approximation
\newcommand{\cx}{c^x}               % coefficients of \hat{x}
\newcommand{\cy}{c^y}               % coefficients of \hat{y}
\newcommand{\Alpha}{\bm{\alpha}}    % set of multi-indices
% B-spline approximation
\newcommand{\cg}{c^g}               % coefficients of g
\newcommand{\ch}{c^h}               % coefficients of h
% Control application
...
\]
The optimal value function $p^\opt:\Pfeas\rightarrow\R$ associates with every $\ppar$ the optimal value of $\Pi(\ppar)$, and the corresponding optimal set is given by the optimal set mapping $X^\opt:\Pfeas\rightarrow 2^{\X}$:
\[ X^\opt(\ppar) = \{x ~:~ \calF(\ppar)x+g(\ppar)\preceq_K 0 ~,~ \langle h(\ppar), x \rangle = p^\opt(\ppar)\} \,.%
\]
An optimizer function $x^\opt:\Pfeas\rightarrow\R^n$ associates to each parameter $\ppar\in\Pfeas$ an optimizer $x^\opt(\ppar)\in X^\opt(\ppar)$.
...
\end{aligned}
\end{gather}
where $\calF(\ppar)^\adj: \Y\rightarrow\X$ denotes the adjoint mapping of $\calF(\ppar)$. The set of all parameter values for which $\Delta(\ppar)$ is feasible is denoted by $\Dfeas$. The dual optimal value function is denoted by $d^\opt$, and the corresponding optimal set mapping by $Y^\opt$.
...
\]
and that both $p^\opt(\ppar)$ and $d^\opt(\ppar)$ are attained (i.e. both $X^\opt(\ppar)$ and $Y^\opt(\ppar)$ are non-empty).
...
\begin{gather}
\hat{x}(\cx) = \sum_{\alpha\in\Alpha^x} \cx_\alpha \, b^x_\alpha \,, \label{eq:x_approx} \\%
\hat{y}(\cy) = \sum_{\alpha\in\Alpha^y} \cy_\alpha \, b^y_\alpha \,, \label{eq:y_approx}%
\end{gather}
\end{subequations}
where $\cx\in\X^{n_x}$ and $\cy\in\Y^{n_x}$, with $n_x=|\Alpha_x|$ and $n_y=|\Alpha_y|$. The images of $\hat{x}(\cx)$ and $\hat{y}(\cy)$ for a particular $\theta$ are indicated by $\hat{x}(\cx,\ppar)$ and $\hat{y}(\cy,\ppar)$, respectively.
...
\begin{aligned}
\hat{\Pi}: && \minimize_{\cx\in\X^{n_x}} &&& \langle h, \hat{x}(\cx) \rangle  \\%
           && \subj                      &&& \calF(\ppar) \hat{x}(\cx,\ppar) + g(\ppar)\preceq_K 0 \;, && \forall \ppar\in\Ppar\;.%
\end{aligned}
\end{gather}
%\begin{gather}\label{eq:semi-inf_primal}
%\begin{aligned}
%\hat{\Pi}: && \minimize_{\cx_\alpha\in\X} &&& \sum_{\alpha\in\Alpha^x} \langle h, \cx_\alpha b^x_\alpha \rangle  \\%
%           && \subj                       &&& \sum_{\alpha\in\Alpha^x} \calF(\ppar) \cx_\alpha\,b^x_\alpha(\ppar) + g(\ppar)\preceq_K 0 \;, && \forall \ppar\in\Ppar\;.%
%\end{aligned}
%\end{gather}
Similarly for the dual:
\begin{gather}\label{eq:semi-inf_dual}
\begin{aligned}
\hat{\Delta}: && \maximize_{\cy\in\Y} &&& \langle g , \hat{y}(\cy) \rangle \\%
...
%\begin{gather}\label{eq:semi-inf_dual}
%\begin{aligned}
%\hat{\Delta}: && \maximize_{\cy_\alpha\in\Y} &&& \sum_{\alpha\in\Alpha^y} \langle g , \cy_\alpha b^y_\alpha \rangle \\%
%              && \subj                       &&& \sum_{\alpha\in\Alpha^y} \calF(\ppar)^* \cy_\alpha\,b^y_\alpha(\ppar) + h(\ppar)= 0 \;, && \forall \ppar \in\Ppar\\%
%              &&                             &&& \sum_{\alpha\in\Alpha^y} \cy_\alpha \, b^y_\alpha(\ppar) \succeq_K 0                  \;, && \forall \ppar \in\Ppar\;.%
%\end{aligned}
%\end{gather}
...
\end{align*}
%\begin{align*}
%\hat{p}(\cx,\ppar) &= \sum_{\alpha\in\Alpha^x} \langle h(\ppar), \cx_\alpha b^x_\alpha(\ppar) \rangle \,,\\%
%\hat{d}(\cy,\ppar) &= \sum_{\alpha\in\Alpha^y} \langle g(\ppar), \cy_\alpha b^y_\alpha(\ppar) \rangle \,  \,.%
%\end{align*}
Then for every feasible point $\cx$ of $\hat{\Pi}$ and every feasible point $\cy$ of $\hat{\Delta}$, the following holds:
\[ \hat{d}(\cy,\ppar) \leq d^\opt(\ppar) \leq p^\opt(\ppar) \leq \hat{p}(\cx,\ppar)\,,\qquad\forall\ppar\in\Ppar\,,%
\]
and the gap between primal and dual objective equals the complementary slackness violation:
\[ \hat{p}(\cx,\ppar) - \hat{d}(\cy,\ppar) = \left\langle \calF(\ppar) \hat{x}(\cx,\ppar) + g(\ppar) \,,\, \hat{y}(\cy,\ppar) \right\rangle \,.%
\]
By solving $\hat{\Pi}$ and $\hat{\Delta}$ the coefficients $\cx$ and $\cy$ are computed that minimize the average gap over $\Ppar$:
\[ \inf_{\cx,\cy} \int_{\Ppar} \hat{p}(\cx,\ppar) - \hat{d}(\cy,\ppar) d\ppar = %
   \inf_{\cx} \int_{\Ppar} \hat{p}(\cx,\ppar) d\ppar - \sup_{\cy} \int_{\Ppar} \hat{d}(\cy,\ppar) d\ppar\,.%
\]
\commentGP{Notation: should we use $\cx_{\text{feas}}$ and $\cy_{\text{feas}}$ to emphasize that we are looking at feasible points?}
...
\noindent\textbf{To include somewhere:}
\begin{itemize}
\item Great approximating power of B-splines. Whereas polynomials form a basis for analytical functions, in most cases neither the optimizer function nor the optimal value function of the considered parametric programs is analytical.
...
from ... concerning the approximation of the stability region of linear
...
\[
\mathbf{\hat{P}} = \{\ppar \in \R^n \, : t^\opt{\star}(\ppar) \geq 0 \}.
\]
...
\[
\mathbf{\bar{P}} = \{\ppar \in \R^n \, : \tr(Z^\opt{\star}(\ppar) P(\ppar)) \leq 0 \}
\]
is found.
...
\appendix
\section{B-spline arithmetic}
Define sum and product of basis functions and ways to determine the linear
mappings from one basis to another...
