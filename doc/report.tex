...
Instead of optimizing worst-case performance, we are actually interested in optimizing $\gamma$ as a function of parameter $\ppar$. This is achieved by solving the convex optimization problem
\begin{eqnarray*}
 ...
 & \left[ \begin{array}{cc} Q(\ppar) & L(\ppar)^T \\ L(\ppar) & Z(\ppar) \end{array} \right] \succ 0.
\end{eqnarray*}
To make the above optimization problem numerically tractable, it is necessary to assume a specific parameterization of the LMI variables, and subsequently use some relaxation technique to derive a finite set of sufficient LMIs. Solving the resulting optimization problem yields an upper bound $\overline{p}(\ppar)$ on the optimal value function $p^{\star}(\ppar)$, while solving the associated dual problem
\begin{eqnarray*}
 ...
 \text{subject to} & \left[ \begin{array}{cc} U_{11}(\ppar) & U_{12}(\ppar) \\ U_{12}(\ppar)^T & U_{22}(\ppar) \end{array} \right] \succ 0, \\
 & V(\ppar) \succ 0, \\
...
\end{eqnarray*}
provides a lower bound $\underline{p}(\ppar)$:
\begin{equation*}
 \underline{p}(\ppar) \leq p^{\star}(\ppar) \leq \overline{p}(\ppar), \quad \ppar \in [0,1].
\end{equation*}
...
\item Pólya vs knot insertion for relaxation
